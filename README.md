...
| **规划与反思** | Plan（规则触发多步分解）→ Act（工具执行）→ Reflect（质检修订） | 规则判定准确；调优记录见 `调试记录_规划与反思.md` |
...
简单闲聊不增加任何开销，只有复杂请求才付出额外调用：

| 阶段 | 触发条件 | 作用 |
| --- | --- | --- |
| **Plan** | 规则判定：含「先/然后/再/顺便/对比…」或一句话两个问号 | 生成 2~5 步执行计划并注入上下文 |
| **Act** | 模型自主决策 | 多轮工具调用直到得出答案 |
| **Reflect** | 用过工具/做过规划必查；普通回答 15% 抽样 | 质检草稿（答非所问/编造/不像人话）并在必要时修订 |

**实测发现（重要）**：首版反思机制是**负收益**——任务达成率 95.7% → 87.0%（↓8.7pp），延迟 1.70s → 2.72s（↑60%）。通过日志定位到两处误判根因并修复：

1. **质检依据被截断**：工具结果只传 120 字符，质检员看不到真实检索内容 → 把正确答案判成"编造"并改坏 → 修复为 1000 字符
2. **质检依据范围过窄**：只给本轮工具结果，模型合理复述历史上下文中的事实被误判为编造 → 新增 `_evidence()` 一并提供系统注入的参考资料，并将判定改为保守策略（不确定一律 PASS）

> 完整排查过程与教训见 [`调试记录_规划与反思.md`](微信版/调试记录_规划与反思.md)。
> 这一节体现的是**用评测数据驱动优化、而非凭观感判断**的工程方法。
...
| **反思机制负收益** | 准确率反而下降 8.7pp | 用评测定位两次误判根因并修复（见第 5 节） |
